...
export class HttpServiceService {
  ...
  getPoks(): Promise<Value[]> {
    const promise = new Promise<Value[]>((resolve, reject) => {
      this.http.get<Value[]>('https://pokeapi.co/api/v2/pokedex/1').subscribe(
        (res: any) => {
          const entries: Value[] = res.pokemon_entries;
          entries.forEach(entry => {
            const temp = new Pokemon();
  ...
  getPok(id: number): Promise<Info> {
    const promise = new Promise<Info>((resolve, reject) => {
  ...
  getDesc(id: number): Promise<Description> {
    ...
    const promise = new Promise<Description>((resolve, reject) => {
